...
{
	FileSelection=gtk_file_selection_new("select background photo");
	gtk_file_selection_set_filename(GTK_FILE_SELECTION(FileSelection),"*.jpg");
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(FileSelection)->ok_button),"clicked",GTK_SIGNAL_FUNC(OpenFile_OfBackgroundTransform),data);
...
void OpenFile(GtkWidget *widget,gpointer *data)
{
	strcpy( position,gtk_file_selection_get_filename(GTK_FILE_SELECTION(FileSelection)) );   
